  ...
  return (
    <AspectWrapper className="bg-[#050811] text-white">
      ...
      >
        ...
        {/* 2. Expansive White Bento / Pinterest Grid (Filling 100% of the screen) */}
        <div
          ref={gridRef}
          className="flex-1 grid grid-cols-4 lg:grid-cols-4 xl:grid-cols-4 gap-2 sm:gap-4 min-h-0"
        >
          {buyerLogos.map((buyer) => (
            <div
              key={buyer.id}
              className="group relative bg-white/95 hover:bg-white rounded-xl sm:rounded-2xl p-2 sm:p-4 shadow-xl border border-white/80 transition-all duration-300 hover:scale-[1.03] hover:shadow-2xl flex items-center justify-center h-full min-h-[42px] sm:min-h-[64px] overflow-hidden"
            >
